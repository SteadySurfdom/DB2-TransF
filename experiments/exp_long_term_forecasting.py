...
def get_trainable_parameter_count(model: nn.Module) -> str:
  trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
  return "{:,}".format(trainable_params) # Format with commas

def print_model_parameter_stats(model):
    print("Model Parameter Statistics:")
    for name, param in model.named_parameters():
        if param.grad is not None:
            grad_norm = param.grad.data.norm(2) #check gradients for any problems
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        if self.args.load_ckpt is not None:
            ...
            saved_model = torch.load(self.args.load_ckpt)
            self.model.load_state_dict(saved_model.state_dict(), strict=False)  
        for epoch in range(self.args.train_epochs):
            print(f'------------The model parameters are: {get_trainable_parameter_count(self.model)}------------')
            iter_count = 0
            train_loss = []
            self.model.train()
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs, aux_loss = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                        else:
                ...
                if self.args.use_amp:
                    ...
                    scaler.unscale_(model_optim)  # Unscale before clipping
        ...
        print(f"Peak GPU Memory Usage: {peak_memory_gb:.3f} GB")
        best_model_path = path + '/' + 'fullmodel.pth'
    ...
    def test(self, setting, test=0):
        test_data, test_loader = self._get_data(flag='test')
        if test:
            print('loading model skibidi')
            self.model = torch.load(os.path.join(self.args.checkpoints, setting, 'fullmodel.pth'))
        MAE = 0
        MSE = 0
        ...
        print('mse:{}, mae:{}'.format(MSE/len(test_loader), MAE/len(test_loader)))
